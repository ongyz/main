...
[NOTE]
Payment list will keep up to 5 payments at any one time. When the 11th payment is added, the payment list will remove the oldest payment record.
[NOTE]
...
The Edit Payment mechanism is facilitated by VersionedTutorHelper and FilteredList<Person>. VersionedTutorHelper extends from TutorHelper and classes related to Payment calls methods from VersionedTutorHelper to retrieve students' data, update them and display accordingly.
