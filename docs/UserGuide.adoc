...
=== Editing payment entries : `paid`

Edits a payment of a specified amount made by a specified student for a specified month and year. +
Format: `paid STUDENT_INDEX AMOUNT MONTH YEAR`

****
* Edits a payment at the specified `STUDENT_INDEX`, for a specified `MONTH` and `YEAR` by increasing it by the specified `AMOUNT`.
* The student index refers to the index number shown in the displayed student list.
* The student index, month and year *must be positive integers* 1, 2, 3, ...
****
...
* `paid 1 200 01 2018` +
`Adds $200 payment made on January 2018 if payment entry already exists
...
=== Edit a syllabus topic from the student’s list of syllabus topics : `editsyll`

Edit a syllabus topic for a specified student and subject. +
